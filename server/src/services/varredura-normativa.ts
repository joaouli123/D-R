import {
  camposPendentesAgente,
  temAnexoNr16ValidoDocumento,
  type AgenteDocumento,
  type CampoPendenteAgente,
  type ItemVarreduraDocumento,
} from './documento-comum.js'

// Espelho: src/lib/varreduraNormativa.ts. Os dois mudam juntos — a tela
// mostra as pendências, a API recusa a emissão com as mesmas regras.
...
export interface VarreduraNormalizadaDocumento {
  nr15: AnexoVarreduraDocumento[]
  nr15Complementares: AnexoVarreduraDocumento[]
  nr16: AnexoVarreduraDocumento[]
}
...
interface CatalogoAnexo {
  ...
  numero: string
  tema: string
  statusFixo?: StatusVarreduraDocumento
}
...
function avaliacaoNr15DoAnexo(agente: AgenteDocumento, anexoId: string): boolean {
...
function avaliacaoNr16DoAnexo(agente: AgenteDocumento, anexoId: string): boolean {
  return agente.tipo === 'periculosidade' && agente.anexoNr16 === anexoId
}

function avaliacaoNr16Concluida(agente: AgenteDocumento): boolean {
...
function montarItens(
  ...
  return catalogo.map((item) => {
    ...
    let status: StatusVarreduraDocumento
...
export function normalizarVarredura(
  tecnico: TecnicoVarreduraDocumento,
  modalidade: string,
): VarreduraNormalizadaDocumento {
...
export function exibirQuadroVarredura(
  tecnico: TecnicoVarreduraDocumento | null | undefined,
...
function anexoNr15DoAgente(agente: AgenteDocumento): CatalogoAnexo | undefined {
...
function anexoNr16DoAgente(agente: AgenteDocumento): CatalogoAnexo | undefined {
  if (!temAnexoNr16ValidoDocumento(agente)) return undefined
...
function quemDaPendencia(pendencia: PendenciaVarreduraDocumento): string {
...
export function descreverAcaoPendencia(pendencia: PendenciaVarreduraDocumento): string {
...
export function descreverPendencia(pendencia: PendenciaVarreduraDocumento): string {
 ...
 */
export function resumirPendencias(pendencias: readonly PendenciaVarreduraDocumento[], limite = 6): string {
  const partes: string[] = []
...
export function mensagemPendencias(pendencias: readonly PendenciaVarreduraDocumento[]): string {
