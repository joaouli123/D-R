...
export interface BlocoQuesitosLaudoDocumento {
  chave: GrupoQuesitosLaudoDocumento['campo'] | 'respostasQuesitos'
...
/**
 * Espelha `blocosQuesitosDoLaudo` de src/lib/quesitosLaudo.ts: os grupos por
 * origem e, por último, o texto do campo antigo (`respostasQuesitos`), que não
 * pode sumir do Laudo de perícias anteriores ao cadastro por origem.
 */
export function blocosQuesitosDoLaudoDocumento(tecnico: TecnicoJson): BlocoQuesitosLaudoDocumento[] {
  const grupos: BlocoQuesitosLaudoDocumento[] = gruposQuesitosDoLaudoDocumento(tecnico).map((grupo) => ({
    chave: grupo.campo,
    titulo: grupo.titulo,
    texto: grupo.texto,
  }))
  ...
  return [
    ...grupos,
    grupos.length
      ? { chave: 'respostasQuesitos', titulo: 'Outras respostas aos quesitos', texto: legado }
